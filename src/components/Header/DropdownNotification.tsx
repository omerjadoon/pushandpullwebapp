...
const DropdownNotification = () => {
  ...
  const [notificationList, setNotificationList] = useState<{ image: string; title: any; subTitle: any; }[]>([]);
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      ...
      const notifications = await Promise.all(
        Object.entries(requestsData || {}).map(async ([id, request]) => {
          const userRef = ref(database, `users/${(request as any).customer_id}`);
