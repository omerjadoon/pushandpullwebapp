...
import {sendCustomerNotification} from '../../../utils/notifications';
...
export default function RequestDetails({ params }: RequestDetailsProps) {
  ...
  const handleAddSuggestion = async () => {
    ...
    try {
      const suggestionsRef = ref(database, `suggestions/users/${customerId}`);
      console.log(newSuggestion);


      
      await push(suggestionsRef, newSuggestion).then(() => {
        console.log("Suggestion added successfully!");
        setSuggestions([...suggestions, newSuggestion]);
      setNewSuggestion({ message: '', slot: '', trainer: '', trainerId: '' });

      
      
      //send notification to customer
      sendCustomerNotification(customerId,newSuggestion.trainerId,"new_suggestion","New Suggestion","Admin has sent you a new suggestion.");
  ...
  const handleUpdateMoreSuggestionsStatus = async (status: string) => {
    ...
    try {
      ...
      //send notification to customer
      sendCustomerNotification(customerId,'notrainer',"more_suggestion_status","Status Updated","Your more suggestions status has been updated to ."+status);
            ...
            <CardContent>
              <div className="flex flex-col space-y-4">
                ...
                <select
                  ...
                  onChange={(e) => {
                    const selectedTrainer = trainers.find(trainer => trainer.displayName === e.target.value);
                    setNewSuggestion({ 
                      ...newSuggestion, 
